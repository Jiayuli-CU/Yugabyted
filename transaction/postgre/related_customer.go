...
func RelatedCustomerTransaction(customerId, warehouseId, districtId uint64) error {
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&postgre.Order{}).Where("customer_id = ? AND warehouse_id = ? AND district_id = ?", customer.Id, customer.WarehouseId, customer.DistrictId).Find(&orders).Error; err != nil {
			log.Printf("Find orders error: %v\n", err)
			return err
		}
		var allOrderLines []postgre.OrderLine
